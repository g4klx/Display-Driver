...
void COLED::writeM17Int(const std::string& source, const std::string& dest, const std::string& type)
{
	m_mode = MODE_M17;
	...
	m_display.setCursor(0, OLED_LINE3);
	m_display.printf("from: %s %s", type.c_str(), source.c_str());

	m_display.setCursor(0, OLED_LINE4);
	m_display.printf("to:   %s", dest.c_str());
...
void COLED::writePOCSAGInt(uint32_t ric, const std::string& message)
{
	...
	if (!rublic.empty())
		m_display.printf(" / %s", rubric.c_str());
